...
Ext.define("Ext.ux.grid.Printer", {
    ...
    statics: {
        ...
        print: function(grid) {
                    ...
                    Ext.each(columns, function(column, col) {
                        ...
                        if (column && column.dataIndex == key) {
							...
							/*if (value && value.indexOf('<span') != -1) { 
								value = ReplaceOne(value, 'color:', 'float:right; color:');								
							}*/
            ...
            var level = [[6, 2, 0],[12, 4, 2, 0]];
            function islevel() {
            	if (grid.id == 'grid_report_product_total_plan' || grid.id == 'grid_report_user_total_plan_1')
            		return 1;
            	if (grid.id == 'grid_report_customer_total_plan')
            		return 0;
            ...
            var nr = 0, rows = [[]];
     		var fn = function (item, index, all) 
     		{
     			if (item.dataIndex == null) 
     			{
     				...
     				{
     					rows[nr].push('<th colspan="' + (islevel()!=-1?level[islevel()][nr]:cols.length) + '"><div align="center">' + item.text + '</div></th>'); 
